...
export default class DonateForm extends Component {
  ...
  onDonateMessageSend = async event => {
    ...
    if(this.state.value === '' || this.state.value === 0) {
      return this.setState({ errorMessage: "You didn't put in an amount of ether!!"});
     }
     if(this.state.value === '' && this.state.description === '' && this.state.name === '') {
      return this.setState({ errorMessage: "Field is empty"});
     }
  try {
    ...
    if(err.message.replace(/ .*/,'') === 'while') {
      return this.setState({ errorMessage: "Numbers only are allowed", value: '',loading: false});
    } else if(err.message.replace(/ .*/,'') === 'Returned') {
      return this.setState({ errorMessage: "Sorry, you cancelled it! Did you mean to do that?", value: '', loading: false});
    }
    if(err.message.replace(/ .*/,'') === 'No') {
      return this.setState({ errorMessage: "Apology! You are not logged in MetaMask wallet", value: '', loading: false});
    }
  }
...
<div className='modal-background'>
  ...
  { this.state.loading===true ? <div className='form-header'> Cogratulations, the nodes are now connecting</div> : <div className='form-header'>Please, Help Our Communities <br/>Grow</div> }
